...
def songInfo():
    ...
    req = requests.get(BASE_URL+artistID[rand_Artist]+'/top-tracks', headers=headers, params=params).json() #GET request for top tracks
    rand_song = random.randint(0,len(req['tracks'])-1) #picks random song from all the songs by certain artist
    albumID=req['tracks'][rand_song]['album']['id']
    albumReq=requests.get('https://api.spotify.com/v1/albums/'+albumID, headers=headers, params=params).json() #GET request for album
    global artistList
    global artistName
    ...
    albumLink=albumReq['external_urls']['spotify']
